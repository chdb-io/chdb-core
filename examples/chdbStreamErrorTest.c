 ...
 *   - an unwritable directory -> a filesystem error (Code 1001)
 *
...
static const char * GENERIC = "Failed to receive table structure";

/* Streaming INSERT init failures must carry the concrete engine error, not
 * the bare generic message that used to mask it. */
static void test_insert_init_error_carries_real_reason(chdb_connection conn)
{
    ...
    CHECK(e2 && strcmp(e2, GENERIC) != 0,
          "unknown-format error is a concrete reason, not the bare generic message");
    if (e2)
...
static void test_materialized_query_error_reason(chdb_connection conn)
{
    ...
    CHECK(e && strcmp(e, GENERIC) != 0, "not the generic streaming message");
...
static void test_streaming_query_error_reason(chdb_connection conn)
{
    ...
    if (!err) {
        ...
        CHECK(err && strcmp(err, GENERIC) != 0, "not the generic streaming message");
