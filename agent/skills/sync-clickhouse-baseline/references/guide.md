...
| --- | --- | --- |
| `EmbeddedServer` | Upstream adds a cache, setting, worker, shutdown call **or registration** only to `LocalServer` | Diff the call vocabulary the release **added** to `LocalServer` against `EmbeddedServer`'s — the whole vocabulary is mostly CLI noise, the added one is short and specific. Cover more than `global_context->set*`: `register*()`, `*Registry::register*`, `get*ThreadPool().initialize`, config fields, start order and stop order. A thread pool is initialized through none of the first two, and v26.9 added two of them. A missing registration compiles fine and fails at run time with an `UNKNOWN_*` error naming something that looks unrelated |
...
| WASM passes every Node test and fails only in the browser, with `Cannot schedule a task` | A browser's pre-spawned Web Worker pool cannot grow under a blocking query; Node's can. So any release that raises peak concurrency fails in exactly one of the two. v26.9's ParquetV3 reader (which replaced the old one and can no longer be switched off) reads the offset index and column chunks concurrently and went past the 16-worker pool | Run `packages/chdb-wasm/test/browser-run.mjs` locally, not just the Node suites — it is the only check that exercises the non-growable pool. The pool size lives in `programs/wasm/CMakeLists.txt` and its comment says to raise it on this error; note that `max_parsing_threads` defaults to the host core count while the pool is fixed, so raising the pool answers the symptom and clamping the setting answers the shape |
| A size or resource gate that passed for years fails on the new baseline | Upstream growth is spread across the engine, and a gate with little headroom left converts it into a hard failure. v26.9 took the Cloudflare lite bundle from 9.31 to 10.66 MiB gzipped against a 9.5 MiB budget, mostly through `DB::Aggregator` (+76% of `Aggregator.h`, a new family of Void/TwoLevel/Hash64 variants) which the lite split force-keeps whole | Attribute before deciding: run `wasm-split --print-profile` against the unsplit module, expand hot + force-kept into a symbol-family breakdown, and compare the families against the sync's own decisions. It separates "upstream grew" from "a resolution added this", which are different problems with different owners. Read such a gate's *number* on every sync, not just its verdict |
| A feature probe stops working on the build that lacks the feature — `UNKNOWN_TABLE`, a missing setting, a missing function — where it used to report the feature as disabled | A release turned a `USE_*`-gated feature fail-closed: instead of existing and refusing, the surface is no longer created at all. v26.9 did this to WebAssembly UDFs, so `system.webassembly_modules` vanishes from any build with `USE_WASMTIME=0`, which is every macOS cross-build (`ENABLE_RUST=0`) and the lite wheel | For each `USE_*` flag a release adds a fail-closed branch to, list the chDB build variants where it is off and ask what each now reports. Code that probes for an optional feature is written against the old wording of its absence |
| `Cannot schedule a task: thread creation unavailable in the single-threaded WASM build` | A release added a scheduling point inside code chDB already adapted for the no-thread build. v26.9's Iceberg reader has two: the producer thread in the constructor, which chDB's `CHDB_WASM_SINGLE_THREADED` branch already replaces, and a per-manifest runner added *inside* `run()`, which it did not | When a release moves work onto a thread pool, grep that file for every scheduling point, not the one an error names. Run the WASM suite against **both** bundles — the threaded one passes either way, so a single-bundle run proves nothing about this |
| A crash at process exit, or a child process writing non-UTF-8 to stderr, after `EmbeddedServer` gained an `initialize()` copied from `LocalServer` | Several `SharedThreadPools` entries lazy-init themselves for non-server binaries — `getDatabaseCatalogShutdownTablesThreadPool` says so at its use site in `DatabasesCommon.cpp`. Adding the call is then not harmless: it changes the pool's configuration and binds a teardown path to a thread pool early | Read the pool's **use site** before copying the call. `initializeWithDefaultSettingsIfNotInitialized()` there means chDB must NOT initialize it; its absence (as in `getIcebergManifestDecodeThreadPool`) means chDB must. The LocalServer/EmbeddedServer diff produces candidates, not decisions |
| `... is not initialized` (LOGICAL_ERROR) from a shared thread pool | The release moved work onto a `SharedThreadPools` entry and initialized it in `LocalServer` only. v26.9 did this to Iceberg manifest decoding, so every Iceberg read failed, and to table shutdown, which only shows up on the way out | Add `get*ThreadPool().initialize` to the `LocalServer` / `EmbeddedServer` comparison; `grep -c` the two files for it and the counts must match |
