...
DataFileEntriesStream::DataFileEntriesStream(
    ...
    /// No producer thread below, so run() fills the queue in one go and must not block on a bound.
    , queue(std::numeric_limits<size_t>::max())
#else
    , queue(queue_size_)
#endif
{
#if defined(CHDB_WASM_SINGLE_THREADED)
    /// Single-threaded WASM build: nothing can be spawned, so produce everything up front.
    try
    {
        run();
    }
