    ...
    if (!tables_snapshot.empty())
    {
        ...
        for (const auto & kv : tables_snapshot)
        {
            ...
            auto run_task = [log_ptr = log, table = kv.second, recorder]
            {
                ...
                /// chdb: the catalog can already be gone when databases are destroyed late
                /// (Python interpreter exit), so check before reaching into it.
                if (table_id->hasUUID() && DatabaseCatalog::isAvailable())
                {
                    try
                    {
                        DatabaseCatalog::instance().removeUUIDMapping(table_id->uuid);
